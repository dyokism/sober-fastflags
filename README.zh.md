...
[Sober](https://sober.vinegarhq.org/) 是一个兼容层，可让 Linux 桌面端原生运行 Roblox Android 应用 (APK)。它以 Flatpak 形式分发 (`org.vinegarhq.Sober`)，默认使用 Vulkan 作为其主要渲染后端，OpenGL 作为备用。配置管理文件位于 `~/.var/app/org.vinegarhq.Sober/config/sober/config.json`。
...
| :--- | :--- | :--- | :--- |
| `DFIntCSGLevelOfDetailSwitchingDistance` | int | `0` - `1000` | CSG 模型的全局 LOD 剔除距离。数值越低 = 帧率（FPS）越高。 |
| `DFIntCSGLevelOfDetailSwitchingDistanceL12` | int | `0` - `1000` | 画质 1–2 时的 LOD 距离。 |
| `DFIntCSGLevelOfDetailSwitchingDistanceL23` | int | `0` - `1000` | 画质 2–3 时的 LOD 距离。 |
| `DFIntCSGLevelOfDetailSwitchingDistanceL34` | int | `0` - `1000` | 画质 3–4 时的 LOD 距离。 |
| `FIntDebugForceMSAASamples` | int | `1`, `2`, `4` | 强制启用 MSAA 抗锯齿（边缘更平滑，但会消耗显卡性能）。 |
| `DFFlagDebugPauseVoxelizer` | bool | `true` / `false` | 暂停体素光照、阴影和环境光遮蔽。大幅提升帧率（FPS）。 |
| `FFlagDebugSkyGray` | bool | `true` / `false` | 将天空盒替换为纯灰色。消除天空着色器（shader）的性能开销。 |
| `DFIntDebugFRMQualityLevelOverride` | int | `0` - `21` | 覆盖画质档位滑块（允许超出默认的 1–10 范围）。 |
| `FIntFRMMaxGrassDistance` | int | `0` - `1000` | 地形草地的最大渲染距离。设置为 `0` 即可禁用草地。 |
| `FIntFRMMinGrassDistance` | int | `0` - `1000` | 草地开始渲染的最近距离。 |
...
图形 API 的选择由 Sober 封装层配置管理，**而非**内部的 FFlags。

- 默认情况下，Sober 使用 **Vulkan** 以获得最佳性能。
- 如果您遇到画面撕裂/伪影、黑屏或启动崩溃（常见于旧显卡或双显卡笔记本），可以通过在 `config.json` 根级设置 `"use_opengl": true` 来强制使用 OpenGL。

不要使用类似 `FFlagDebugGraphicsPreferVulkan` 或 `FFlagDebugGraphicsPreferOpenGL` 的引擎 FFlags——当它们与 Sober 容器级的 API 选择冲突时，会导致上下文不匹配并发生崩溃。
...
| Flag | 弃用原因 |
| :--- | :--- |
| `DFIntTaskSchedulerTargetFps` | 已替换为直接修改 `GlobalBasicSettings_13.xml` 的方法。 |
| `FFlagTaskSchedulerLimitTargetFpsTo2402` | 已从白名单中移除。 |
| `DFIntConnectionMTUSize` | 网络调优相关的 flag 已被屏蔽。 |
| `FFlagDebugDisableTelemetryEphemeralCounter` | 遥测阻止相关的 flag 已被屏蔽。 |
| `FFlagAdServiceEnabled` | 广告服务开关已被屏蔽。 |
| `FFlagMovePrerender` | 多线程调配相关的 flag 已被屏蔽。 |
| `DFIntDebugDynamicRenderKiloPixels` | 渲染分辨率缩放功能已被 Roblox 工程团队否决。 |
...
**官方来源：**[Allowlist for local client configuration via Fast Flags — Roblox DevForum](https://devforum.roblox.com/t/allowlist-for-local-client-configuration-via-fast-flags/3966569)
