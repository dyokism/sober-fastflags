...
[O Sober](https://sober.vinegarhq.org/) é uma camada de compatibilidade que executa o aplicativo Roblox Android (APK) nativamente em desktops Linux. Ele é distribuído como um Flatpak (`org.vinegarhq.Sober`) e usa o Vulkan como seu backend de renderização principal, com o OpenGL como alternativa (fallback). A configuração é gerenciada através de `~/.var/app/org.vinegarhq.Sober/config/sober/config.json`.
...
| :--- | :--- | :--- | :--- |
| `DFIntCSGLevelOfDetailSwitchingDistance` | int | `0` - `1000` | Distância principal de culling LOD para modelos CSG. Menor = melhor FPS. |
...
| `DFFlagDebugPauseVoxelizer` | bool | `true` / `false` | Pausa a iluminação de voxels, sombras e oclusão ambiental. Grande aumento de FPS. |
| `FFlagDebugSkyGray` | bool | `true` / `false` | Substitui o skybox por um cinza plano. Remove o processamento extra de shaders de céu. |
| `DFIntDebugFRMQualityLevelOverride` | int | `0` - `21` | Substitui o controle deslizante de nível gráfico (vai além do padrão 1–10). |
| `FIntFRMMaxGrassDistance` | int | `0` - `1000` | Distância máxima de renderização para grama do terreno. Defina como `0` para desativar a grama. |
...
A seleção da API gráfica é gerenciada pelas configurações do wrapper do Sober, **não** por FFlags internas do jogo.

- Por padrão, o Sober usa **Vulkan** para obter o melhor desempenho.
- Se você tiver problemas de artefatos visuais, telas pretas ou crashes na inicialização (comum em GPUs antigas ou notebooks híbridos), force o uso do OpenGL definindo `"use_opengl": true` no nível raiz do seu `config.json`.

Não use FFlags internas como `FFlagDebugGraphicsPreferVulkan` ou `FFlagDebugGraphicsPreferOpenGL` para isso — elas podem causar crashes de incompatibilidade de contexto ao entrarem em conflito com a seleção de API do Sober.
...
| :--- | :--- |
| `DFIntTaskSchedulerTargetFps` | Substituída pela edição do arquivo `GlobalBasicSettings_13.xml`. |
| `FFlagTaskSchedulerLimitTargetFpsTo2402` | Removida da allowlist. |
| `DFIntConnectionMTUSize` | Flags de otimização de rede estão bloqueadas. |
| `FFlagDebugDisableTelemetryEphemeralCounter` | Supressão de telemetri é bloqueada. |
| `FFlagAdServiceEnabled` | Bloqueio do serviço de anúncios é bloqueado. |
| `FFlagMovePrerender` | Flags de manipulação de threads estão bloqueadas. |
| `DFIntDebugDynamicRenderKiloPixels` | A alteração de escala de resolução de renderização foi vetada pela equipe de engenharia do Roblox. |
...
**Fonte Oficial:** [Allowlist for local client configuration via Fast Flags — Roblox DevForum](https://devforum.roblox.com/t/allowlist-for-local-client-configuration-via-fast-flags/3966569)
